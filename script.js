...
function render(){ if(!state.team) return renderStart(); renderMission(); }
function navButtons(){
...
function buildInputArea(m){
  ...
  if(m.type==='letters'){
    ...
    [...m.solution].forEach(ch=>{ if(ch===' '){const g=document.createElement('span');g.className='gap';row.appendChild(g);return;} const input=document.createElement('input'); input.className='letter'; input.maxLength=1; input.inputMode='text'; input.autocomplete='off'; input.dataset.char=ch; wireInput(input); row.appendChild(input); });
...
function focusFirst(){ const i=allInputs()[0]; if(i) setTimeout(()=>i.focus(),300); }
